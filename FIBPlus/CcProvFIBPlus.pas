...
var
  q: TCcQueryFIB;
begin
  q := TCcQueryFIB.Create(Self, qry, nID, qry.SelectStatement);
...
constructor TCcQueryFIB.Create(Conn:TCcConnection; qry: TCcQuery; nID: Integer; Select: Boolean);
begin
	inherited;
...
destructor TCcQueryFIB.Destroy;
begin
//  FIBQuery.Free;
  inherited;
end;
...
procedure TCcConnectionFIB.CreateDatabase;
begin
  SetConnectParams;
  FIBDatabase.DBParams.Clear;
  FIBDatabase.DBParams.Add('USER ''' + UserLogin + ''' PASSWORD ''' + UserPassword + '''');
...
procedure TCcConnectionFIB.SetConnectParams;
begin
  FIBDatabase.DBParams.Clear;
...
begin
	if not FIBTransaction.InTransaction then begin
...
function TCcConnectionFIB.GetConnectorConnected: Boolean;
begin
	Result := FIBDatabase.ExTestConnected(laCloseConnect);
//	Result := FIBDatabase.Connected;
end;

function TCcConnectionFIB.GetInTransaction: Boolean;
begin
