...
uses Classes, SysUtils, DB, CcProviders, CcInterbaseConn, UIB, UIBLib;
...
uses UIBase, CcInterbase {$IFDEF CC_D6}, Variants{$ENDIF};
...
begin
  inherited;
  IBDatabase := TUIBDatabase.Create(Self);
  LibraryName := IBDatabase.LibraryName;

  IBTransaction := TUIBTransaction.Create(Self);
  IBTransaction.DataBase := IBDatabase;

  FOptions := [tpWrite,tpNowait];
...
var
  q: TCcQueryUIB;
begin
	q := TCcQueryUIB.Create(Self, qry, nID, qry.SelectStatement);
...
constructor TCcQueryUIB.Create(Conn: TCcConnection; qry: TCcQuery; nID: Integer; Select: Boolean);
begin
  inherited;
	IBQuery := TUIBQuery.Create(Conn);
end;
...
begin
  ...
  if field.IsBlob[index] then
    Result := ftBlob
  else if field.IsBlobText[index] then
    Result := ftMemo;
  ...
  case VarType(Val) of
    ...
    varOleStr, varString:
      SQLDA.AsString[Field.Index] := Val;
		{$IFDEF CC_D2K9}
		varUString:
			SQLDA.AsUnicodeString[Field.Index] := Val;
    {$ENDIF}
		{$IFDEF CC_D6}
		varInt64:
...
procedure TCcConnectionUIB.CreateDatabase;
begin
  IBDatabase.DatabaseName := DBName;
  IBDatabase.Params.Values['lc_ctype'] := CharSet;
  IBDatabase.Params.Values['Password'] := UserPassword;
  ...
  IBDatabase.SQLDialect := SQLDialect;
  IBDatabase.LibraryName := LibraryName;
  IBDatabase.CreateDatabase(csNONE);
end;
...
function TCcConnectionUIB.GetConnectorConnected: Boolean;
begin
  Result := IBDatabase.Connected;
end;

function TCcConnectionUIB.GetInTransaction: Boolean;
begin
