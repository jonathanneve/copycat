...
uses
  ...
  ComCtrls, Buttons, ExtCtrls, CcProviders, CcInterbaseConn, CcProvFIBPlus;
...
type
  TfmMain = class(TForm)
    ...
    ConnectionFIB: TCcConnectionFIB;
    edLocalPassword: TEdit;
    Label6: TLabel;
...
procedure TfmMain.ReplicatorAbort(Sender: TObject);
begin
...
begin
  ...
  if not ConnectionFIB.Connected then begin
    Screen.Cursor := crHourGlass;
    try
      with ConnectionFIB do begin
        ...
        if Trim(edClientDLL.Text) <> '' then
          ClientDLL := edClientDLL.Text
        else
          ClientDLL := 'gds32.dll';
      end;
      Replicator.Nodes.LocalNode.Connection := ConnectionFIB;
      Replicator.Nodes.LocalNode.Password := edLocalPassword.Text;
...
begin
  frRemoteParams.Init;
  cbVersions.Items.Assign(ConnectionFIB.DBAdaptor.SupportedVersions);
//  frLocalParams.Init;
end;
