...
-------------------------
- Create TCcXmlParser Instance                     MyXml := TCcXmlParser.Create;
...
"LoadFromBuffer" loads the internal buffer of TCcXmlParser with the given null-terminated
...
buffer pointer must be valid while the document is accessed via TCcXmlParser.
===============================================================================================
Encodings:
----------
...
The "TranslateEncoding" method that is built into TCcXmlParser assumes that the application
...
- The TCcXmlParser instance can either "own" the buffer itself (then, FBufferSize is > 0)
...
Whitespace Handling
-------------------
The TCcXmlParser property "PackSpaces" determines how Whitespace is returned in Text Content:
...
Please note that TCcXmlParser does _not_ normalize Line Breaks to single LineFeed characters
as the XmlSpec requires (XmlSpec 2.11).
The xml:space attribute is not handled by TCcXmlParser. This is on behalf of the application.
===============================================================================================
Non-XML-Conforming
------------------
TCcXmlParser does not conform 100 % exactly to the XmlSpec:
- UTF-16 is not supported (XmlSpec 2.2)
  (Workaround: Convert UTF-16 to UTF-8 and hand the buffer over to TCcXmlParser)
...
2000-03-28  HeySt  1.0.1   Initial Publishing of TCcXmlParser on the destructor.de Web Site
2000-03-30  HeySt  1.0.2   TCcXmlParser.AnalyzeCData: Call "TranslateEncoding" for CurContent
...
2000-07-23  HeySt  1.0.8   TCcXmlParser.Clear: CurAttr.Clear; EntityStack.Clear;
...
2001-02-28  HeySt  1.0.10  Completely worked over and tested the UTF-8 functions
                           Fixed a bug in TCcXmlParser.Scan which caused it to start over when it
...
2002-06-26  HeySt  1.0.16  TCcXmlParser.Scan: Fixed a bug with PIs whose name is beginning
...
TYPE
  ...
  TDtdElementRec = RECORD    // --- This Record is returned by the DTD parser callback function
                     ...
                     ElementType : TDtdElemType;                       // Type of the Element
  ...
  TCcXmlParser = CLASS                             // --- Internal Properties and Methods
               PROTECTED
               ...
               PUBLIC
                 ...
                 FUNCTION  LoadExternalEntity (SystemId, PublicId,
                                               Notation : STRING) : TCcXmlParser;     VIRTUAL;
  ...
  TEntityStack = CLASS (TObjectList)    // Stack where current position is stored before parsing entities
                 PROTECTED
                   Owner : TCcXmlParser;
                 PUBLIC
                   CONSTRUCTOR Create (TheOwner : TCcXmlParser);
...
FUNCTION  AnsiToUtf8  (Source : {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF}) : STRING;                            // Convert Win-1252 to UTF-8
FUNCTION  Utf8ToAnsi  (Source : STRING; UnknownChar : Char = '¿') : {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF};  // Convert UTF-8 to Win-1252


(*
===============================================================================================
TCustomXmlScanner event based component wrapper for TCcXmlParser
===============================================================================================
*)
...
TYPE
  ...
  TCustomXmlScanner = CLASS (TComponent)
    ...
    PROTECTED
      PROPERTY XmlParser           : TCcXmlParser        READ FXmlParser;
...
FUNCTION  AnsiToUtf8 (Source : {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF}) : STRING;
...
FUNCTION  Utf8ToAnsi (Source : STRING; UnknownChar : Char = '¿') : {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF};
...
BEGIN
  ...
  WHILE I <= SourceLen DO BEGIN
    ...
    IF A < $80 THEN BEGIN                                               // Range $0000..$007F
      INC (Len);
      {$IFDEF CC_D2K9} // Delphi 2009 or later
      Result [Len] := {$IFDEF MSWINDOWS}AnsiChar{$ELSE}Char{$ENDIF}( Source [I]);
      {$ELSE}
      Result [Len] := Source [I];
      {$ENDIF}
      INC (I);
    ...
    ELSE BEGIN                                                          // Determine U, Inc I
      ...
      INC (Len);
      {$IFDEF CC_D2K9} // Delphi 2009 or later
      Result [Len] := {$IFDEF MSWINDOWS}AnsiChar{$ELSE}Char{$ENDIF}( Ch);
      {$ELSE}
      Result [Len] := Ch;
      {$ENDIF}
      END;
...
Encoding found there is used for the External Entity (is assigned to TCcXmlParser.CurEncoding)
...
CONSTRUCTOR TEntityStack.Create (TheOwner : TCcXmlParser);
BEGIN
  INHERITED Create;
  Owner := TheOwner;
END;
...
CONSTRUCTOR TCcXmlParser.Create;
BEGIN
  INHERITED Create;
  FBuffer     := NIL;
  FBufferSize := 0;
...
DESTRUCTOR TCcXmlParser.Destroy;
BEGIN
  Clear;
  Elements.Free;
  Entities.Free;
...
FUNCTION  TCcXmlParser.LoadFromFile (Filename : STRING; FileMode : INTEGER = fmOpenRead OR fmShareDenyNone) : BOOLEAN;
          // Loads Document from given file
...
FUNCTION  TCcXmlParser.LoadFromBuffer (Buffer : PChar) : BOOLEAN;
...
PROCEDURE TCcXmlParser.SetBuffer (Buffer : PChar);      // References another buffer
BEGIN
  Clear;
  FBuffer     := Buffer;
  FBufferSize := 0;
...
PROCEDURE TCcXmlParser.StartScan;
BEGIN
  CurPartType := ptNone;
  CurName     := '';
  CurContent  := '';
...
FUNCTION  TCcXmlParser.Scan : BOOLEAN;
          // Scans the next Part
...
PROCEDURE TCcXmlParser.AnalyzeProlog;
...
PROCEDURE TCcXmlParser.AnalyzeComment (Start : PChar; VAR Final : PChar);
          // Analyze Comments
BEGIN
...
PROCEDURE TCcXmlParser.AnalyzePI (Start : PChar; VAR Final : PChar);
...
PROCEDURE TCcXmlParser.AnalyzeDtdc;
...
PROCEDURE TCcXmlParser.AnalyzeDtdElements (Start : PChar; VAR Final : PChar);
...
PROCEDURE TCcXmlParser.AnalyzeTag;
          // Analyze Tags
VAR
  S, F    : PChar;
  Attr    : TAttr;
...
PROCEDURE TCcXmlParser.AnalyzeCData;
...
PROCEDURE TCcXmlParser.AnalyzeText (VAR IsDone : BOOLEAN);
...
PROCEDURE TCcXmlParser.AnalyzeElementDecl  (Start : PChar; VAR Final : PChar);
...
PROCEDURE TCcXmlParser.AnalyzeAttListDecl  (Start : PChar; VAR Final : PChar);
...
PROCEDURE TCcXmlParser.AnalyzeEntityDecl   (Start : PChar; VAR Final : PChar);
...
PROCEDURE TCcXmlParser.AnalyzeNotationDecl (Start : PChar; VAR Final : PChar);
...
PROCEDURE TCcXmlParser.PushPE (VAR Start : PChar);
...
PROCEDURE TCcXmlParser.ReplaceCharacterEntities (VAR Str : STRING);
...
PROCEDURE TCcXmlParser.ReplaceParameterEntities (VAR Str : STRING);
...
PROCEDURE TCcXmlParser.ReplaceGeneralEntities (VAR Str : STRING);
...
FUNCTION  TCcXmlParser.LoadExternalEntity (SystemId, PublicId, Notation : STRING) : TCcXmlParser;
          // This will be called whenever there is a Parsed External Entity or
          // the DTD External Subset to be parsed.
          // It has to create a TCcXmlParser instance and load the desired Entity.
...
BEGIN
  ...
  // --- Load the File
  Result := TCcXmlParser.Create;
  Result.LoadFromFile (Filename);
END;


FUNCTION  TCcXmlParser.TranslateEncoding  (CONST Source : STRING) : STRING;
...
PROCEDURE TCcXmlParser.DtdElementFound (DtdElementRec : TDtdElementRec);
...
FUNCTION TCcXmlParser.GetDocBuffer: PChar;
...
===============================================================================================
TScannerXmlParser
A TCcXmlParser descendant for the TCustomXmlScanner component
===============================================================================================
*)

TYPE
  TScannerXmlParser = CLASS (TCcXmlParser)
                       Scanner : TCustomXmlScanner;
                       CONSTRUCTOR Create (TheScanner : TCustomXmlScanner);
                       FUNCTION  LoadExternalEntity (SystemId, PublicId,
                                                     Notation : STRING) : TCcXmlParser;  OVERRIDE;
...
FUNCTION  TScannerXmlParser.LoadExternalEntity (SystemId, PublicId, Notation : STRING) : TCcXmlParser;
BEGIN
