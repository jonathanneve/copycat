...
uses
  ...
  CcConf, CcReplicator, ExtCtrls, Buttons, DB,
  CcProvFireDAC, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  ...
  Vcl.DBGrids, FireDAC.Comp.DataSet, CcMemDS, CcDB, FireDAC.Phys.MSSQL,
...
begin
  // LocalConnection := XXX;
  // RemoteConnection := YYY;

//  FDConnection1.Params.Values['Database'] := 'localhost:' + ExtractFileDir(Application.ExeName) + '\DB1.fdb';
//  FDConnection2.Params.Values['Database'] := 'localhost:' + ExtractFileDir(Application.ExeName) + '\DB2.fdb';
  LocalConnection := CcConnectionFireDAC1;
  RemoteConnection := CcConnectionFireDAC2;
...
begin
  ...
  Replicator.Direction := sdRemoteToLocal;
  Replicator.Replicate;
end;
...
begin
  ...
  Replicator.Direction := sdBoth;
  Replicator.BatchImport('DOSSIERS', sdRemoteToLocal, '0=0', '0=0', False);
end;
...
begin
  ...
  for i := 0 to clbTablesRemote.Items.count - 1 do
  begin
    if clbTablesRemote.Checked[i] then
    begin
...
begin
  qTest.Close;
  if CcConnectionFireDAC1.DBAdaptor.QuoteMetadata then
    qTest.Macro['upper_case'].Value := ''
  else
    qTest.Macro['upper_case'].Value := 'upper';
  qTest.Param['config_name'].Value := 'DEMO';
  qTest.Exec;
  while (not qTest.Eof) do begin
    cTest := qTest.Field['TRACKED_FIELDS'].AsString;
    qTest.Next;
  end;
end;
...
begin
  ...
  for i := 0 to Replicator.Log.Keys.count - 1 do
  begin
    if primaryKeys <> '' then
...
procedure TMainForm.ReplicatorBeforeReplicate(Sender: TObject);
begin
  count := 0;
  nStart := GetTickCount;
end;
...
begin
  ...
  Inc(nRowCount);
  if nRowCount mod 100 = 0 then
    memLog.Lines.Add(IntToStr(nRowCount) + ' rows replicated in ' + IntToStr(GetTickCount - nStart)+ ' seconds');
...
begin
nRowCount := 0;
  if not Assigned(LocalConnection) or not Assigned(RemoteConnection) then
  begin
    ShowMessage('You need to set the LocalConnection and RemoteConnection variables to point to your database connection components in the FormShow method!');
