...
uses
  ...
  CcProviders, CcProvIBX, ExtCtrls, CcConf, IBDatabase, Db, IBServices,
  IBSQL, IBCustomDataSet, CcConfStorage, CcReplicator, Grids, DBGrids,
  DBCtrls, StdCtrls, ComCtrls, Buttons;
...
type
  TForm1 = class(TForm)
    ...
    Splitter1: TSplitter;
    Splitter2: TSplitter;
    CcProviderIBX1: TCcConnectionIBX;
    qLocalTables: TIBSQL;
    ...
    procedure ReplicatorRowReplicating(Sender: TObject; TableName: string;
      Fields: TFields; var CanContinue: Boolean);
    ...
    procedure ReplicatorGenReplicating(Sender: TObject);
...
procedure TForm1.ReplicatorRowReplicating(Sender: TObject; TableName: string;
  Fields: TFields; var CanContinue: Boolean);
var
  cMessage:String;
begin
  // *** Update/simplify this code using new parameters above?
...
procedure TForm1.ReplicatorGenReplicating(Sender: TObject);
begin
  // *** Rewrite this code since Name and NewValue parameters are no longer available?
...
var
  cAppDir:String;
begin
  if not (Assigned(Replicator.Nodes.LocalNode.Connection) and Assigned(RplConfig.Connection)) then begin
...
//Replicator.Users.LocalSYSDBA.Password := cSYSDBAPassword;
  Replicator.LocalNode.Connection.UserLogin := Trim(edLocalUser.Text);
  Replicator.LocalNode.Connection.UserPassword := 'CopyCat';
  Replicator.RemoteNode.Connection.UserLogin := Trim(edRemoteUser.Text);
  Replicator.RemoteNode.Connection.UserPassword := 'CopyCat';
...
begin
  ...
  RplConfig.Connection.CharSet := Replicator.LocalDB.CharSet;
//RplConfig.Connection.SQLDialect := Replicator.LocalDB.SQLDialect;
  RplConfig.Connection.DBName := Replicator.LocalDB.DBName;
  RplConfig.Connection.UserLogin := cSYSDBA;
  RplConfig.Connection.UserPassword := cSYSDBAPassword;
  RplConfig.Connect();
  qLocalTables.Close;
  ...
  RplConfig.Disconnect();
  RplConfig.Connection.CharSet := Replicator.RemoteDB.CharSet;
//RplConfig.Connection.SQLDialect := Replicator.RemoteDB.SQLDialect;
  RplConfig.Connection.DBName := Replicator.RemoteDB.DBName;
  RplConfig.Connection.UserLogin := cSYSDBA;
  RplConfig.Connection.UserPassword := cSYSDBAPassword;
  RplConfig.Connect();
  qRemoteTables.Close;
