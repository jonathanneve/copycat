...
function MimeEncodeString(const S: {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF}): {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF};

{ @abstract(Mime-encodes an AnsiString.)
  @Name is just like @link(MimeEncodeString), but does <B>not</B> insert line breaks. }
function MimeEncodeStringNoCRLF(const S: {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF}): {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF};
...
function MimeDecodeString(const S: {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF}): {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF};
...
{$IFDEF MSWINDOWS}
procedure DecodeHttpBasicAuthentication(const BasicCredentials: {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF}; out UserId, PassWord: {$IFDEF MSWINDOWS}AnsiString{$ELSE}String{$ENDIF});
{$ENDIF}
