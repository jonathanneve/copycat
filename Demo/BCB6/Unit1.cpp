...
{
  LogEvent("Replication started: " + String(Replicator->Log->LineCount) + String(" rows"));
...
void __fastcall TForm2::RplConfigQueryReady(TObject *Sender,
      TStrings *Script)
{
  qScript->Database = ((TCcDatabaseIBX*)RplConfig->DB)->IBDatabase;
  qScript->Transaction = ((TCcTransactionIBX*)RplConfig->TR)->IBTransaction;
...
//---------------------------------------------------------------------------
void __fastcall TForm2::GrantTable(TIBDatabase* DB, TIBTransaction* TR, String cTableName)
{
  qGrant->Close();
  qGrant->Database = DB;
...
void __fastcall TForm2::TimerTimer(TObject *Sender)
{
  ...
  if (qGetLocalUser->FieldByName("LOGIN")->AsString.Trim() != "")
    edLocalUser->Text = qGetLocalUser->FieldByName("LOGIN")->AsString.Trim();

  qGetRemoteUser->Close();
  qGetRemoteUser->ExecQuery();
  if (qGetRemoteUser->FieldByName("LOGIN")->AsString.Trim() != "")
    edRemoteUser->Text = qGetRemoteUser->FieldByName("LOGIN")->AsString.Trim();
}
//---------------------------------------------------------------------------
...
//      GrantTable(((TCcDatabaseFIBPlus*)RemoteDB)->FIBDatabase, ((TCcTransactionFIBPlus*)RemoteTR)->FIBTransaction, "CUSTOMERS");
...
//      GrantTable(((TCcDatabaseFIBPlus*)LocalDB)->FIBDatabase, ((TCcTransactionFIBPlus*)LocalTR)->FIBTransaction, "CUSTOMERS");
...
//---------------------------------------------------------------------------
void __fastcall TForm2::btHelpClick(TObject *Sender)
{
...
//  qLocalDataSet->Database = ((TCcDatabaseFIBPlus*)Database)->FIBDatabase;
//  qLocalDataSet->Transaction = ((TCcTransactionFIBPlus*)Database->Transaction)->FIBTransaction;
}
//---------------------------------------------------------------------------
void __fastcall TForm2::LocalDBLogin(TIBDatabase *Database,
      TStrings *LoginParams)
{
  if (cSYSDBAPassword == "") {
    fmDBLogin = new TfmDBLogin(this);
    if (fmDBLogin->ShowModal() == mrOk) {
